...
//    cinemetaApiV3: 'https://v3-cinemeta.strem.io/meta',
    ...
    staleThreshold: 3 * 24 * 60 * 60 * 1000,            // 3 days - lazy refresh threshold
    ...
    // DOM Selectors
    domSelectors: {
        containers: '.meta-items-container-n8vNz, .meta-items-container-qcuUA, .meta-items-container-IKrND',
...
window.MetadataModules.config = {
    ...
    init: function() {
        ...
        const loadScript = () => {
            if (!document.head) {
                console.warn('[Metadata Helper] document.head not available, retrying in 1s');
            ...
            script.onload = () => console.log('[Metadata Helper] Dexie.js loaded from CDN');
            script.onerror = () => console.warn('[Metadata Helper] Failed to load Dexie.js');
